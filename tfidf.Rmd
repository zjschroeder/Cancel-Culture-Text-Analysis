...
terms_Dfm2<- dfm_trim(terms_Dfm, min_docfreq = 0.001, 
                      docfreq_type = "prop")

terms_Dfm3 <- dfm_subset(terms_Dfm2, 
                         ntoken(terms_Dfm2) > 0) 

data_terms_clean <- dfm_tfidf(terms_Dfm3)

terms_cosine <- textstat_simil(data_terms_clean, 
                               margin = "features", 
                               method = "cosine")

terms_g <- graph_from_adjacency_matrix(as.matrix(terms_cosine),
                                       mode = "undirected", 
                                       weighted = TRUE, 
                                       diag = FALSE) 
```
