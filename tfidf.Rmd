...
#scatterplot
    LIBandCONSERV <- ggplot(data_scatter, aes(x = libcos , y = concos, label = rownames(feature1))) + geom_abline(intercept = 0, slope = 1) + 
      geom_label_repel(size =6, aes(color = partisanlean, label=feature1, hjust=0, vjust=0), data=data_scatter, max.overlaps = getOption("ggrepel.max.overlaps", default = 100), segment.size = 0, label.padding = .3, box.padding = .3, point.padding = .5, force = 2) + scale_color_gradient2(midpoint = mid, low = "navyblue", mid = "grey50", high = "darkred", space = "Lab" )
